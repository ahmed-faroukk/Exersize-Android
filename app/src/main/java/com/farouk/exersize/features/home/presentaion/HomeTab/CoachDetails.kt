...
class CoachDetails(private val id : String) : Screen {
...
fun CoachDetailsUi(viewModel: HomeViewModel , id : String){
    ...
    coach.data?.let {
        CoachScreenContent(it){
            navigator.push(PackagesScreen(id , "${it.msg.fname} ${it.msg.lname}" , "${HomeApiInterface.BASE_URL}${it.msg.personal_img}" , it.msg.exp.toString()))
        }
    }
}
